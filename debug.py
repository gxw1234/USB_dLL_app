...
# 定义设备详细信息结构体 - 更新为支持DLL和STM32分离的信息
class DeviceInfoDetail(Structure):
    _fields_ = [
        # DLL信息
        ("DllName", c_char * 32),           # DLL名称字符串
        ("DllBuildDate", c_char * 32),      # DLL编译时间字符串
        ("DllVersion", c_int),              # DLL版本号
        
        # STM32设备固件信息
        ("FirmwareName", c_char * 32),      # 设备固件名称字符串
        ("FirmwareBuildDate", c_char * 32), # 设备固件编译时间字符串
        ("HardwareVersion", c_int),         # 硬件版本号
        ("FirmwareVersion", c_int),         # 设备固件版本号
...
def main():
    ...
    if info_result == 0:
        ...
        print(f"  DLL编译时间: {dev_info.DllBuildDate.decode('utf-8')}")
        ...
        print(f"  固件编译时间: {dev_info.FirmwareBuildDate.decode('utf-8')}")
        # 硬件版本解析：高字节.低字节
        hw_major = (dev_info.HardwareVersion >> 8) & 0xFF
        hw_minor = dev_info.HardwareVersion & 0xFF
        print(f"  硬件版本: {hw_major}.{hw_minor}")
        # 固件版本解析：高字节.低字节
        fw_major = (dev_info.FirmwareVersion >> 8) & 0xFF
        fw_minor = dev_info.FirmwareVersion & 0xFF
        print(f"  固件版本: {fw_major}.{fw_minor}")
        print("\n=== 设备信息 ===")
