...
def main():
    ...
    # 定义SPI_SlaveReadBytes函数参数和返回值类型
    usb_application.SPI_SlaveReadBytes.argtypes = [c_char_p, c_int, POINTER(c_ubyte), c_int]
    usb_application.SPI_SlaveReadBytes.restype = c_int
    ...
    if handle >= 0:
        ...
        if spi_init_result == SPI_SUCCESS:
            ...
            # ===================================================
            print("\n开始读取SPI数据...")
            read_buffer_size = 21  # 读取缓冲区大小
            read_buffer = (c_ubyte * read_buffer_size)()
            
            # 循环读取SPI数据
            for i in range(5):  # 读取5次作为测试
